...
interface UserReport {
  ...
  reportedUser: { _id: string; name: string; username: string };
  reason: string;
  description?: string;
  createdAt: string;
}
...
export default function AdminUserReportsPage() {
  ...
  const { data, mutate, isLoading } = useSWR(
    accessToken ? '/api/reports/user' : null,
    fetcher
  );

  const reports: UserReport[] = data?.data?.reports || data?.reports || [];
  const unresolvedCount = data?.data?.unresolvedCount ?? data?.unresolvedCount ?? reports.length;

  const handleResolve = async (reportId: string) => {
    setActionLoading(reportId);
    try {
      const res = await fetchWithAuth(`/api/reports/user/${reportId}/resolve`, {
        method: 'PUT',
      });
      if (res.ok) {
  ...
  const filteredReports = reports.filter((r: UserReport) =>
    ...
    r.reportedUser?.username?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Review and resolve reported users.
            {unresolvedCount > 0 && (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/50 border-b border-border">
                ...
                <th className="px-4 sm:px-6 py-4 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground hidden sm:table-cell">Reported User</th>
            ...
            <tbody className="divide-y divide-border/60">
              {isLoading ? (
                ...
                filteredReports.map((r: UserReport) => (
                  <tr key={r._id} className="hover:bg-muted/20 transition-colors group">
                    ...
                    <td className="px-4 sm:px-6 py-4 hidden sm:table-cell">
                      <p className="text-sm font-medium text-foreground truncate">{r.reportedUser?.name}</p>
                      <p className="text-[10px] text-muted-foreground truncate">@{r.reportedUser?.username}</p>
                    </td>
                    ...
                    <td className="px-4 sm:px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <a
                          href={`/profile/${r.reportedUser?.username}`}
                          target="_blank"
