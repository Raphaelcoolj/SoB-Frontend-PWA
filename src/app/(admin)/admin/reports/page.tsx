...
interface PostReport {
  ...
  post: { _id: string; title?: string; body: string; author: { name: string; username: string } };
  reason: string;
  description?: string;
  createdAt: string;
}
...
export default function AdminPostReportsPage() {
  ...
  const { data, mutate, isLoading } = useSWR(
    accessToken ? '/api/reports' : null,
    fetcher
  );

  const reports: PostReport[] = data?.data?.reports || data?.reports || [];
  const unresolvedCount = data?.data?.unresolvedCount ?? data?.unresolvedCount ?? reports.length;

  const handleResolve = async (reportId: string) => {
    setActionLoading(reportId);
    try {
      const res = await fetchWithAuth(`/api/reports/${reportId}/resolve`, {
        method: 'PUT',
      });
      if (res.ok) {
  ...
  const filteredReports = reports.filter((r: PostReport) =>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/50 border-b border-border">
                ...
                <th className="px-4 sm:px-6 py-4 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground hidden sm:table-cell">Post</th>
            ...
            <tbody className="divide-y divide-border/60">
              {isLoading ? (
                ...
                filteredReports.map((r: PostReport) => (
                  <tr key={r._id} className="hover:bg-muted/20 transition-colors group">
                    ...
                    <td className="px-4 sm:px-6 py-4 hidden sm:table-cell max-w-[200px]">
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-foreground truncate">{r.post?.title || 'No title'}</p>
                        <p className="text-xs text-muted-foreground line-clamp-1">{r.post?.body}</p>
                      </div>
                    </td>
