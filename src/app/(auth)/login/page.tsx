 ...
 * Authenticates users using email and password, validating inputs via Zod.
 * Connects directly to the backend auth login API.
 * Includes fallback link for password reset and Google OAuth button.
 */
...
// Validation schema matching login requirements
...
export default function LoginPage() {
  ...
  const { login, loading } = useAuth(); // removed error from hook since we use toast
  ...
  // Connects redirect to OAuth backend
  ...
  return (
    <Card className="backdrop-blur-md bg-card/90 shadow-2xl border-border/80">
      <CardHeader className="text-center space-y-1">
        ...
        <CardDescription>Log in to access your SoB account</CardDescription>
      </CardHeader>
      <CardContent>
